...
\textit{Lean 4 Implementation (\texttt{Phase5\_GlobalSection.lean}):} We apply sheaf theory to prove that phase-locked local fields fuse into a single global section, via \texttt{global\_section\_from\_thermodynamics}. Two caveats. First, phase-locking is not derived here: it is the assumed class field \texttt{ThermodynamicCover.thermodynamic\_equilibrium}. Second, and more seriously, \emph{no instance of \texttt{ThermodynamicCover} has been constructed}, so the theorem is conditional on a structure not yet shown to be realizable; building one requires explicit sections of the sheafified probability presheaf. We therefore claim a formalized gluing mechanism, not a solved binding problem.
