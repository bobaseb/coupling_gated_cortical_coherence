...
* **The code must still carry what the input does not.** A readout that returns
  a state stored for each input (`stored`) is a contraction with `Λ = 0`,
...
def map (u : U) (s : S) : S := Enc.readout u (Enc.encode s)

/-- The reconstruction error at a state, under that state's own input. -/
def error (s : S) : ℝ := dist s (Enc.map (Enc.input s) s)
...
theorem dist_le_of_conditioned_contracting [PseudoMetricSpace U] {q L : ℝ≥0} (hq : q < 1)
    {ε : ℝ} (hmap : ∀ u, LipschitzOnWith q (Enc.map u) Enc.relevant)
    (hin : ∀ u v, ∀ s ∈ Enc.relevant, dist (Enc.map u s) (Enc.map v s) ≤ L * dist u v)
  ...
  set u := Enc.input s
  set v := Enc.input t
  have hsplit := dist_triangle4 s (Enc.map u s) (Enc.map u t) t
  have hmid : dist (Enc.map u t) t ≤ dist (Enc.map u t) (Enc.map v t) + Enc.error t := by
    have := dist_triangle (Enc.map u t) (Enc.map v t) t
    rwa [dist_comm (Enc.map v t) t] at this
  ...
  have herr : dist s (Enc.map u s) = Enc.error s := rfl
...
theorem dist_le_of_same_input [PseudoMetricSpace U] {q L : ℝ≥0} (hq : q < 1) {ε : ℝ}
    (hmap : ∀ u, LipschitzOnWith q (Enc.map u) Enc.relevant)
    (hin : ∀ u v, ∀ s ∈ Enc.relevant, dist (Enc.map u s) (Enc.map v s) ≤ L * dist u v)
    (hrec : Enc.Reconstructs ε) {s t : S} (hs : s ∈ Enc.relevant) (ht : t ∈ Enc.relevant)
...
theorem encode_injOn_fibre {ε : ℝ} (hrec : Enc.Reconstructs ε) {u : U} {F : Set S}
  ...
  simp only [error, map, (hF hs).2, (hF ht).2] at hes het
...
theorem dist_le_of_encode_const (hE : ∀ s t, Enc.encode s = Enc.encode t) {ε : ℝ}
  ...
  have hes := hrec s hs
  have het := hrec t ht
  simp only [error, map, ← hu] at hes het
  linarith
...
`stored` returns a state kept for each input and ignores the code. Its map is
...
def stored (relevant : Set S) (input : S → U) (store : U → S) : ConditionedEncoding S U Unit where
  relevant := relevant
  input := input
  encode := fun _ => ()
...
theorem stored_reconstructs_iff {relevant : Set S} {input : S → U} {store : U → S} {ε : ℝ} :
    (stored relevant input store).Reconstructs ε ↔
...
theorem stored_contracting {relevant : Set S} {input : S → U} {store : U → S} (u : U) :
    LipschitzWith 0 ((stored relevant input store).map u) :=
...
noncomputable def selfState (hc : ∀ u, ContractingWith q (Enc.map u)) (u : U) : S :=
  ContractingWith.fixedPoint (Enc.map u) (hc u)
...
theorem dist_selfState_le [PseudoMetricSpace U] (hc : ∀ u, ContractingWith q (Enc.map u))
    (hin : ∀ u v s, dist (Enc.map u s) (Enc.map v s) ≤ L * dist u v) (u v : U) :
    dist (Enc.selfState hc u) (Enc.selfState hc v) ≤ L * dist u v / (1 - q) :=
  (hc u).dist_fixedPoint_fixedPoint_of_dist_le' (Enc.map v) (hc u).fixedPoint_isFixedPt
...
its input selects. -/
theorem dist_selfState_input_le (hc : ∀ u, ContractingWith q (Enc.map u)) {ε : ℝ}
