...
\begin{abstract}
\singlespacing
Coordinated cortical activity need not describe a single experienced situation. We develop a conditional field account that distinguishes collective timing, compatibility of local descriptions and an internal representation of the resulting state. The physical hypothesis is that changing extracellular geometry increases field-mediated coupling during awakening. A noisy mean-field phase model gives a square-root stationary onset, but finite ramps delay the response. Increasing coupling, decreasing diffusion and a matched common drive can also produce identical stationary phase distributions, so recovery measurements must resolve dynamics and distinguish the interaction mechanism. The representational problem is separate: an explicit counterexample has equal phases with incompatible local states, and a plasticity control preserves coherence while losing environmental alignment. These results show what coordination alone leaves undetermined. We propose the state glued from compatible spatial measures as a formal correlate of unity, and its accurate internal reconstruction as a correlate of a minimal self. Lean~4 checks the conditional composition under eight connecting hypotheses and additional structures. The construction establishes uniqueness for specified local data and a contracting map; identifying those objects with meaningful neural representations remains open. The account motivates tests of how cortical coordination maintains agreement about shared content and whether an internal representation of that same state is required for unified experience.
\end{abstract}
\linenumbers
...
The positive proposal begins at that limit. Compatible spatial measures determine one global state; an encoding within that state can reconstruct it through a specified readout. We argue that these relations capture aspects of unity and an internal perspective, while examining why reconstruction by itself is insufficient for the psychological interpretation. Lean~4 checks the composition under explicit connecting hypotheses and additional structures, with toy witnesses showing that the assumptions can hold together. The proofs establish what follows from those choices; the main unresolved work is to identify their neural counterparts and explain how coordination maintains them. Figure~\ref{fig:chain} locates these proposed connections, and Section~\ref{sec:chain} states their physical and thermodynamic assumptions.
...
We identify the section glued from compatible local states with the unity of a conscious episode, and its accurate internal reconstruction with a minimal self. The motivation concerns two features of the episode: different contents belong to one situation, and that situation is experienced from a perspective. The mathematical relations give these proposals a definite form; their psychological interpretation needs a further argument.
...
This motivates the reflexive condition but does not make accurate reconstruction sufficient for a perspective. A recording of a scene can be reconstructed, and a local signal can covary with a body's position, without either being used by the system as a representation of its own condition. The proposed interpretation needs evidence that the encoding participates in how the system distinguishes changes in itself from changes in its surroundings. In the cup example, changing the encoded hand or viewpoint while holding the external scene fixed should have consequences for the system's estimates of its relation to the cup that follow from the proposed readout. This is a criterion for assigning self-representational meaning to the variables; it is not a consequence of $F(s_*)=s_*$.

The fixed-point construction contributes a consistency requirement to that interpretation: the internal representation must be accurate about the very state that contains it. Its uniqueness prevents ambiguity for the chosen map, but does not establish that the representation is used, that it specifies a perspective, or that a perspective is experienced. A biological model must connect the encoding to those functions before the equality becomes evidence about a minimal self.
...
The reason to propose reflexivity as necessary is the thought that even an unreflective episode has a perspective: the cup appears somewhere relative to the perceiver, whether or not the perceiver attends to themselves. Moving from that feature of experience to a requirement for internal self-encoding is the philosophical hypothesis. Gluing alone does not require it. Unified experience without the proposed encoding would count against the reflexive requirement while leaving the compatibility proposal assessable on its own.
...
The relevant cover and map must be tied to neural organization by evidence independent of the desired conclusion. Local descriptions should predict the contents attributed to their domains, and their restrictions should preserve meaning on shared domains. The encoding and readout should be specified before testing them, with their sensitivity to changes in the proposed state measured over a declared range. Exact reconstruction at the candidate state is the fixed-point condition; fidelity during changes requires its own assessment.

Contraction and a fixed point are easy to obtain in an uninformative construction. A constant readout returning one predetermined state satisfies those conditions. Restriction makes the source of the encoding explicit, but a readout can still discard what it receives. The cup example would require sensitivity to the represented hand and viewpoint, and the intervention described above would test whether that sensitivity has the proposed role. Mathematical existence alone cannot distinguish a self-model from a reconstruction chosen by an observer.
...
An implementation of the proposed representations must retain information and update it through physical processes. The supporting results bound finite-register capacity by $H(p)\leq\log|X|$ and erasure heat under a specified system--bath model \cite{landauer1961,norton2005,shenker2000}. A separate field-boundary result concerns a continuous field reaching disconnected vacuum components \cite{kibble1976}. These constrain one proposed architecture; the stationary phase threshold and the representational argument do not depend on deriving that architecture from finite capacity. The supplement develops these results and their witnesses.
...
\begin{equation}
 ...
 \label{eq:predictive}
\end{equation}
Under the no-back-action Markov structure $X_t\to S_t\to S_{t+1}$, the bracket is nonnegative \cite{still2012}. At a given dissipation budget, the bound limits retained information that fails to predict the next signal. Zero retained information can satisfy it, so it supplies no incentive to learn the cup's location or maintain an accurate self-model. Connecting this budget to Landauer heat requires the nonpredictive information to be bounded by erased entropy at the same thermal scale and heat budget. Capacity, erased entropy and predictive information thus impose different constraints \cite{rosas2026}; content and learning dynamics must still be specified.

\subsection{What the composition establishes}

Table~\ref{tab:summary} lists the eight implications accepted by the theorem \texttt{chain}. They connect the supporting constraints to the phase model and its proposed representational role. Each connection is a hypothesis; in particular, the finite register and the field boundary are independent starting objects.
...
Two structures also enter the theorem: a thermodynamic cover, including overlap compatibility, and a reflexive boundary specifying the local encoding and readout. Given these and the connecting hypotheses, \texttt{chain} concludes \texttt{UnifiedSelf}: the section glued on that cover is the unique fixed point of self-prediction. Restriction resonance, needed for the interpretation in Eq.~\eqref{eq:selfmap}, is a separate condition. The result binds the fixed point to the specified local states; compatibility and accuracy at that state are premises, while uniqueness follows from the construction.

The joint witness combines a double well, a register, a uniform-grid mesh and a three-site cortex, establishing that the hypotheses can hold together in distinct toy components. It supplies no common biological mechanism for them. Counterexamples expose the role of individual assumptions, including overlap agreement. The development declares no additional Lean axioms beyond the foundational axioms used by its libraries; physical premises are explicit hypotheses and model fields.
...
The proposed account takes a further step by identifying a compatible, internally represented state with a conscious episode. The cup example illustrates its appeal: descriptions of appearance and action can contribute to one situation, with the perceiver represented within it. The main explanatory gap is the mechanism that would maintain those relations. Compatibility is assumed, and the map's accuracy at the glued state is assumed. The proofs establish their consequences and joint realizability in toy components; they do not yet explain why cortical activity should acquire or preserve them.
...
Testing the self clause requires a further choice: a candidate encoding whose contribution to representing the system's own condition can be perturbed. The readout should predict which aspects of the represented scene, body and viewpoint change when that encoding changes. Evidence that it merely reconstructs data would leave the perspective interpretation unsettled; evidence of its proposed functional role would make dissociations from unified experience informative. The exact fixed-state idealization must also be assessed against the fidelity and time dependence of that biological process.
