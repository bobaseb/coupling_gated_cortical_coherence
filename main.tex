...
under local detailed balance, with $Q$ positive into the reservoir \cite{horowitz2014,ito2013}. This supplies the thermodynamic constraint for the active branch below. A proved two-substep extension composes actuation with a memory update initialized from its coordinate-swapped final law. It identifies the output with the perception--action channels and cancels the same intermediate entropy and expected energy. At a common thermal scale, the whole update's joint entropy decrease is bounded by its total heat budget divided by $k_BT$, without assuming stationarity. The supplement gives the balances and reciprocal two-bit witnesses with explicit interaction energy, heat and work. Action and observation alias the existing states; additional physical registers require further accounting. A biological objective, learning rule and cortical action readout remain modelling choices.
...
E34 / E34Active & Bridge assumption & Passive: a predictive structure with the register's thermal scale and heat budget, and nonzero wasted memory. Active: positivity, local detailed balance and the register's heat allocation for the named elementary process. The proved two-substep bound supplies no allocation. \\
